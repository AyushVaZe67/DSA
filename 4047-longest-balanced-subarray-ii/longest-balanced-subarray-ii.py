    ...
    def longestBalanced(self, nums: List[int]) -> int:
        ...
        class SegmentTree:
            def __init__(self, data):
                ...
                self.build(data, 1, self.n, 1)

            def apply_tag(self, i, tag):
                self.tree[i].min_value += tag.to_add
                self.tree[i].max_value += tag.to_add
                self.tree[i].lazy_tag += tag

            def pushdown(self, i):
                if self.tree[i].lazy_tag.has_tag():
                    tag = self.tree[i].lazy_tag
                    self.apply_tag(i << 1, tag)
                    self.apply_tag(i << 1 | 1, tag)
                    self.tree[i].lazy_tag.clear()

            def pushup(self, i):
                self.tree[i].min_value = min(
                    self.tree[i << 1].min_value,
                    self.tree[i << 1 | 1].min_value,
                )
                self.tree[i].max_value = max(
                    self.tree[i << 1].max_value,
                    self.tree[i << 1 | 1].max_value,
                )

            def build(self, data, l, r, i):
                if l == r:
                    self.tree[i].min_value = data[l - 1]
                    self.tree[i].max_value = data[l - 1]
                    return
                mid = l + ((r - l) >> 1)
                self.build(data, l, mid, i << 1)
                self.build(data, mid + 1, r, i << 1 | 1)
                self.pushup(i)

            def update(self, target_l, target_r, tag, l, r, i):
                if target_l <= l and r <= target_r:
                    self.apply_tag(i, tag)
                    return
                self.pushdown(i)
                mid = l + ((r - l) >> 1)
                if target_l <= mid:
                    self.update(target_l, target_r, tag, l, mid, i << 1)
                if target_r > mid:
                    self.update(target_l, target_r, tag, mid + 1, r, i << 1 | 1)
                self.pushup(i)

            def add(self, l, r, val):
                if l > r:
                    return
                self.update(l, r, LazyTag(val), 1, self.n, 1)

            def find(self, target_l, target_r, val, l, r, i):
                ...
                mid = l + ((r - l) >> 1)
                if target_r >= mid + 1:
                    res = self.find(target_l, target_r, val,
                                    mid + 1, r, i << 1 | 1)
                    if res != -1:
                        return res
                if l <= target_r and mid >= target_l:
                    return self.find(target_l, target_r, val,
                                     l, mid, i << 1)
                return -1
            ...
            def find_last(self, start, val):
                if start > self.n:
                    return -1
                return self.find(start, self.n, val, 1, self.n, 1)
